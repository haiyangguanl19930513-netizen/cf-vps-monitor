...
- **Ping 监控**：支持 ICMP、TCP、HTTP Ping 任务，可分配到全部节点或指定节点，并展示延迟历史；内置四川电信、联通、移动的一键配置，节点卡片显示近一小时延迟与丢包率。
...
1. 本次升级不修改数据库结构，部署完成后无需执行数据库迁移。
2. 进入后台 `延迟监测`，点击 `添加四川三网`，系统会为全部服务器补充四川电信、联通、移动 ICMP 任务；已存在的目标不会重复添加。
3. 探针收到新任务并上报后，首页节点卡片会显示最近一小时的平均延迟与估算丢包率，通常等待 1～2 分钟即可看到首批数据。
